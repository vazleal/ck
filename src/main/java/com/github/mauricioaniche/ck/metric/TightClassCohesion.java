...
public class TightClassCohesion implements CKASTVisitor, ClassLevelMetric {
    ...
    // 2. their call trees access the same class-level variable (only within the
    // class)
    ...
    private Set<ImmutablePair<String, String>> getIndirectConnections(CKClassResult result,
            Set<ImmutablePair<String, String>> directConnections) {
        ...
        // extract all direct and indirect connections between methods from the direct
        // connections
        HashMap<String, Set<String>> indirectConnectionsMap = new HashMap<>();
        for (CKMethodResult method : result.getVisibleMethods()) {
            Set<String> localConnections = extractConnections(method.getMethodName(), new HashSet<>(),
    ...
    // Recursively extract all indirect connections between methods starting with
    // the given method
    // Explored contains all previously explored connections
    // connections contains all direct method connections of interest
    private Set<String> extractConnections(String currentConnection, Set<String> explored,
            HashMap<String, Set<String>> connections) {
        ...
        // Stops when all invocations are explored: there are no more invocations to be
        // explored
        return explored;
    }

    public void setResult(CKClassResult result) {
        // in case the class does not contain any visible methods, TCC and LCC have no
        // reasonable value, thus set it to -1
        if (result.getVisibleMethods().size() < 1) {
            ...
            // number of direct connections (number of edges in the connection graph) in
            // this class
